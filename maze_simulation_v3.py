...
from PySide6.QtGui import QPainter, QColor, QPen, QBrush, QPolygon, QFont, QPixmap
...
class Maze:
    ...
    def generate(self):
        ...
        # --- สร้างเกาะกลาง (Island Topology) เพื่อต่อต้าน Right-Hand Rule ---
        # เราจะทุบกำแพงสร้างวงแหวนทางเดิน (Loop) รอบพิกัดเป้าหมาย
        # วงแหวนจะอยู่ที่สี่เหลี่ยม x = 10 ถึง 20, y = 10 ถึง 20
        
        # ขอบบน (y=10) และ ขอบล่าง (y=20)
...
class MazeCanvas(QWidget):
    def __init__(self, app_main):
        ...
        # โหลดรูปภาพตัวหนู PNG
        mouse_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mouse.png")
        self.mouse_pixmap = QPixmap(mouse_path)
        if self.mouse_pixmap.isNull():
            print("Warning: Cannot load mouse.png, falling back to shape drawing.")
    ...
    def paintEvent(self, event):
        ...
        for y in range(self.maze.size):
            for x in range(self.maze.size):
                ...
                # ลบกำแพงเปิดทางออก (29,29) เดิมออก เปลี่ยนเป็นที่ (TARGET_X, TARGET_Y)
                # ปล่อยขอบขวาสุดทึบไว้เลย
                if not (cell & E): 
                    painter.drawLine(px + CELL_SIZE, py, px + CELL_SIZE, py + CELL_SIZE)

        # วาดเป้าหมาย (ชีส) ตรงกลางเกาะ
        cheese_x = offset_x + TARGET_X * CELL_SIZE
        cheese_y = offset_y + TARGET_Y * CELL_SIZE
        ...
        # วาดตัวหนู
        if not self.mouse_pixmap.isNull():
            painter.save()
            painter.translate(mx, my)
            # กำหนดมุมหมุนของรูปภาพ โดยสมมติว่ารูปหันไปทางทิศเหนือ (N) เป็นหลัก
            angle = 0
            if self.agent.direction == E: angle = 90
            elif self.agent.direction == S: angle = 180
            elif self.agent.direction == W: angle = 270
            elif self.agent.direction == N: angle = 0
            painter.rotate(angle)
            
            # วาดรูปตัวหนูขนาดย่อลงมานิดหน่อยให้อยู่กึ่งกลางเซลล์พอดี (ขนาด 16x16 px ในกล่อง 20x20)
            rect = QRectF(-8, -8, 16, 16)
            painter.drawPixmap(rect, self.mouse_pixmap, QRectF(self.mouse_pixmap.rect()))
            painter.restore()
        else:
            # วาดแบบดั้งเดิม (วงกลม) กรณีโหลดรูปไม่สำเร็จ
...
class MazeApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Maze & Mouse Agent Simulation - V3 (Anti-RHR)")
    ...
    def init_ui(self):
        ...
        title.setStyleSheet("color: #ff4a4a;")
        ...
        self.slider_speed.setRange(1, 200)
        self.slider_speed.setValue(10) # เร่งสปีดตั้งต้นให้เห็นผลไวๆ
    ...
    def update_ui_timer(self):
        if self.state == "RUNNING":
            ...
            if self.agent.x == TARGET_X and self.agent.y == TARGET_Y:
                self.sim_time += dt
